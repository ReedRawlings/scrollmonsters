...
(async () => {
  ...
  try {
    ...
    assert.equal((await read()).party.y, 180);
    ...
    s = await read(); assert(Math.abs(s.party.y - 292) < 0.001); assert.equal(s.party.x, 270);
    assert.equal(s.combat.cameraScroll, 0); assert.equal(s.combat.movementMode, 'party-advances-static-ground');
