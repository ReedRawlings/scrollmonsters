...
await p.goto('http://localhost:5174/upgrade-prototype.html');await p.waitForTimeout(500);fs.mkdirSync('output/upgrade-prototype',{recursive:true});
...
await p.evaluate(()=>window.__scollTest.setSave({gold:1000,essence:{feral:100,bloom:100,arcane:100}}));await tap(270,700);await tap(390,780);
...
await p.locator('canvas').screenshot({path:'output/upgrade-prototype/player.png'});
await tap(270,702);assert((await p.evaluate(()=>window.__scollTest.getSave())).gold<before.gold);
for(let i=1;i<5;i++){await tap(62+i*104,150);await p.locator('canvas').screenshot({path:`output/upgrade-prototype/branch-${i}.png`});}
